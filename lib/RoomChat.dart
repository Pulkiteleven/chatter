...
class roomChat extends StatefulWidget {
  String roomId;
  String roomName;
  roomChat({Key? key,required this.roomId,required this.roomName}) : super(key: key);
...
class _roomChatState extends State<roomChat> {
    ...
    await ref.onValue.listen((event) {
      if (event.snapshot.value != null) {
        ...
        if (data != null) {
          ...
          for (var x in a) {
            print("superman ${chatData[x]}");
            Future.delayed(Duration(seconds: 1), () {
              scroolDown();
            });
            if(chatData[x]['msg'] != null){
    ...
    return MaterialApp(
      home: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: mainText(widget.roomName, Colors.white, 15.0, FontWeight.normal,1),
        ),
        body: Stack(
  ...
  Send() async {
    ...
    ref.child('chats').child(widget.roomId).update(sendMsg).then((value) => {
      setState(() {
        msg = "";
        fieldText.clear();
        scroolDown();
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          // )),
          (!isUser)?Avatar(widget.id, 15.0):Container(),
